...
>
> **0.1.5–0.1.6** — fixes two client-side defects found in use. The textarea value used to live in component state: when the settings mirror never reported the namespace as `ready`, reopening Settings showed an empty box while the saved value stayed active on the host — the text looked lost and could not be edited. The text now lives in the row's slot store (which survives the dialog closing), and every mounting also reads the value straight from the Host through `remote.settings.describe()`, so the saved prompt is visible even when the mirror stays silent; both read paths are revision-guarded and never overwrite unsaved edits. The debounced autosave also cancelled its pending write on unmount, dropping an edit made within 600 ms of closing; it now flushes on unmount. An explicit **Save** button and an "unsaved changes" indicator were added alongside the autosave.
