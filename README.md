...
>
> **0.1.5** — fixes two client-side defects found in use. The textarea value used to live in component state: when the settings mirror never reported the namespace as `ready`, reopening Settings showed an empty box while the saved value stayed active on the host — the text looked lost and could not be edited. The text now lives in the row's slot store (which survives the dialog closing) and is only replaced by an `adopt` action when nothing is unsaved. The debounced autosave also cancelled its pending write on unmount, dropping an edit made within 600 ms of closing; it now flushes on unmount. An explicit **Save** button and an "unsaved changes" indicator were added alongside the autosave.
